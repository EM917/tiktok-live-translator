...
商品知识按主播和品牌分两层叠在全局表上面：`profiles/<主播用户名>.txt` 是某个
主播专属的写法与译法，按直播间地址自动加载；`brands/<品牌 id>.txt` 是「这一场
只卖这一个牌子」时才加载的品牌词表，在开始页「本场品牌」下拉里选（给没有自己
profile、但整场只卖同一个牌子的主播用）。两者都是首次使用时从同名
`*.example.txt` 模板生成可编辑副本，改完「停止」再「开始」即生效；同一个写法
三层都有时，主播 profile 优先于品牌，品牌优先于全局表。
