...
- <a id="fault-tolerance"></a>🔄 **抗故障** —— 最多七层的流地址解析链路，按顺序依次尝试，每层各自隔离，一层出错不会拖累其它层：**登录直播页**（macOS，借用浏览器里已有的 TikTok 登录）→ **官方接口**（不经过 yt-dlp，也是唯一给纯音频档的一层）→ **登录直播页重试**（只在第一次带登录抓页读到了可用登录、却没拿到地址，且官方接口没确认下播时才走）→ **系统 WebKit 引擎加载直播页**（macOS，TikTok 只把某些房间的流地址给真正的浏览器时靠它，实测 2 秒）→ **yt-dlp 匿名** → **yt-dlp 借用浏览器登录态** → **直播页兜底**（同样先带登录抓、匿名的那次放最后）——因为 yt-dlp 提取器被拦截时会将失败报告为「未开播」。未指定浏览器（`--cookies-browser` 或 `cookies_browser_only`）时，登录直播页这一步只读取 Safari——其它浏览器只在匿名路径均未取到地址之后、由后面的借登录层读取；Safari 中有登录时不再读取其它浏览器的数据。登录直播页重试会先等 3 秒，或者中间发过匿名请求时按下面的 8 秒规则等完差额，再重新发同一次带登录请求，排在较慢的几层之前；一旦拿到地址，解析就在这一层结束。实测：部分直播间只向已登录观众提供流地址，且紧跟在匿名请求之后 3 秒内的登录请求取不到地址，程序因此在两者之间留出 8 秒（2026-09-17 的一次重连实录：同一房间当天更早的每一次解析都是 0.6–1.4 秒就拿到地址，这一次登录直播页在 0.5 秒读到页面却没有地址，WebKit 超时 25 秒，两次 yt-dlp 都报未开播，同一种带登录抓页方式在 31 秒时才拿到地址，全程共 37.6 秒；第一次为什么没有地址不知道）。`http://` 开头的 TikTok 链接按 `https://` 解析，登录信息不经明文传输。评论连接在第一次解析返回之后才开始，因为它的第一个请求是对同一页面的匿名请求。读取 Safari 的 cookie 需要为本程序授予「完全磁盘访问权限」（见自检「浏览器登录态」一行）；读不到登录时监听照常开始、按匿名方式解析，并在界面上持续提示处理步骤。每个解析结果在会话开始前先行验证。断流后以重新解析的地址重连，并区分网络中断与直播结束；识别落后时自动丢段保实时；后台保持 yt-dlp 为最新版。以上都没拿到地址时，程序会隔 20 秒自动重试几次，仍不行就明确提示；这类直播间可以把**直播间链接和浏览器里的 .flv 地址一起粘进输入框**（空格隔开），前者管弹幕与词表、后者原样当作音频源，直接跳过上面的整条解析链路——实测这种签名地址有效期约两周，取一次够用整场。监听期间阻止系统自行进入睡眠；程序整段没有运行时会记录并在界面上说明。重连前先探测网络，断网不消耗重连次数；接口没说「已结束」的房间最多等主播回来 10 分钟
...
```mermaid
flowchart TD
    URL["TikTok 直播间地址"] --> RESOLVE["解析直播流<br/>登录直播页（macOS）→ 官方接口 → 重试 → WebKit（macOS）→ yt-dlp → 借登录态 → 直播页<br/>（优先纯音频档，见「抗故障」）"]
...
**违禁词警示默认是关的。** 每次点「开始」前，开播面板里有一个「开启违禁词警示」
开关，默认不勾选。不勾选时检测照常进行、命中照常写入审计（多带一个
`suppressed: "alerts_off"` 标记），只是不会弹报警、不发系统通知、不占用强模型
重译；要让命中弹出来必须显式打开这个开关。上次的选择会记住，下次开播默认沿用。
...
报告为「主播未开播」，该结论不成立——完整的分层顺序和实测耗时见上文
[抗故障](#fault-tolerance)。在 Chrome 或 Safari 中登录 TikTok 有帮助但通常并非必需，
...
并在终端里说明。这条只讲控制面：手机同看用的是另一个、不会自动漂移的端口，见下一条。

**同事能用手机看吗？** 可以。在首页「手机同看」卡片点「打开」，手机连上和这台
电脑同一个 Wi-Fi，扫二维码或手工输入卡片上的地址即可；手机端只能看字幕和报警，
不能操作本程序——开始、停止、切引擎、改设置都不行。最多同时 12 台手机在看。
链接里带着一把钥匙，当密码看待：发给谁，谁就能看到字幕和报警；点「换一个链接」
会让当前在看的手机全部断开，需要重新扫码。手机打不开时，程序这边只知道没有
连接进来，可以依次试：确认手机连的是同一个 Wi-Fi；换卡片列表里另一个地址手工
...
限速（两次点击间要隔几秒，每分钟最多几次），一台手机连点也不会堆积任务、拖慢
