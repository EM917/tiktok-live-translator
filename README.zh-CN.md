...
- 🔄 **抗故障** —— 五条独立的流地址解析路径（TikTok 官方接口 → 系统 WebKit 引擎在后台加载直播页（macOS，TikTok 只把某些房间的流地址给真正的浏览器时靠它，实测 2 秒）→ yt-dlp → yt-dlp 借用浏览器登录态 → 直播页解析），因为 yt-dlp 提取器被拦截时会将失败报告为「未开播」。macOS 上读得到浏览器中的 TikTok 登录时，程序发出的第一个请求是携带该登录的直播页，未取到地址才走上述五条路径（实测部分直播间只向已登录观众提供流地址，且紧跟在匿名请求之后 3 秒内的登录请求取不到地址，程序因此在两者之间留出 8 秒）；未指定浏览器（`--cookies-browser` 或 `cookies_browser_only`）时，这一步只读取 Safari——其它浏览器只在匿名路径均未取到地址之后、由后面原有的借登录路径读取；Safari 中有登录时不再读取其它浏览器的数据。`http://` 开头的 TikTok 链接按 `https://` 解析，登录信息不经明文传输。评论连接在第一次解析返回之后才开始，因为它的第一个请求是对同一页面的匿名请求。读取 Safari 的 cookie 需要为本程序授予「完全磁盘访问权限」（见自检「浏览器登录态」一行）；读不到登录时监听照常开始、按匿名方式解析，并在界面上持续提示处理步骤。每个解析结果在会话开始前先行验证。断流后以重新解析的地址重连，并区分网络中断与直播结束；识别落后时自动丢段保实时；后台保持 yt-dlp 为最新版。TikTok 不把某个直播间的流地址给程序时，程序会隔 20 秒自动重试几次，仍不行就明确提示；这类直播间可以把**直播间链接和浏览器里的 .flv 地址一起粘进输入框**（空格隔开），前者管弹幕与词表、后者作音频源——实测这种签名地址有效期约两周，取一次够用整场。监听期间阻止系统自行进入睡眠；程序整段没有运行时会记录并在界面上说明。重连前先探测网络，断网不消耗重连次数；接口没说「已结束」的房间最多等主播回来 10 分钟
- 💬 **观众弹幕翻译** —— 评论由程序自己通过 TikTokLive 从直播间评论流获取（WebSocket 签名经第三方 Euler Stream 服务，这是本项目唯一不在本机完成的环节；需要 Python 3.10+，首次开播自动安装组件；一般免登录，TikTok 要求登录时会借用浏览器里的 TikTok 登录态重试一次）。译文显示在本地界面的弹幕面板里，只翻译、只显示，不进报警链路。弹幕只在当前翻译引擎是本地模型（Hy-MT2 1.8B 或 TranslateGemma）时翻译，用远程引擎或 7B 时面板只显示原文。可用 `--no-comments` 关闭
...
界面在**独立应用窗口**中打开（不占浏览器标签页），上次填过的直播间地址和选过的目标语言都会记住，点「开始翻译」即可；输入框下方还列着**最近打开过的直播间**（只显示主播名），点一下就直接开始。关掉窗口就是退出。想改回浏览器界面加 `--browser`。
...
或通过 `--cookies cookies.txt` 提供凭据。如需长期只读取某一个浏览器，在
`settings.json` 中设置 `"cookies_browser_only": "safari"`；显式给出的
`--cookies-browser` 优先于该设置。
...
7B 翻译模型加载耗时 42–60 秒。MLX 会把释放的缓冲留在缓存中以便复用，程序默认不为
该缓存设上限。可通过环境变量 `TLT_MLX_CACHE_MB`，或 `settings.json` 中的
`"mlx_cache_limit_mb"` 设定上限（环境变量优先）；取值为非负整数，单位 MB，`0` 表示
不保留缓存。默认行为未作改动：设定上限对识别延迟的影响尚未测量，而识别延迟处于
报警的关键路径上。该设置仅对 `mlx` 后端生效，在识别模型加载时读取，修改后需重新
...
**提示「TikTok 不把这个直播间的流地址给程序（代码 4003110）」。** 这是 TikTok 的
通用拒绝码，返回体里没有原因，程序也不替它编一个。下结论之前先跑内置的检查：

```bash
python3 tools/diagnose_room.py @主播名
```
