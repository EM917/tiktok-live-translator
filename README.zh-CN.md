...
- 🔄 **抗故障** —— 六条独立的流地址解析路径（TikTok 官方接口 → 系统 WebKit 引擎在后台加载直播页（macOS，TikTok 只把某些房间的流地址给真正的浏览器时靠它，实测 2 秒）→ yt-dlp → yt-dlp 借用浏览器登录态 → 直播页解析 → 已登录的 Chrome，经插件取播放器实际使用的流地址），因为 yt-dlp 提取器被拦截时会将失败报告为「未开播」。每个解析结果在会话开始前先行验证。断流后以重新解析的地址重连，并区分网络中断与直播结束；识别落后时自动丢段保实时；后台保持 yt-dlp 为最新版。有些直播间 TikTok 只把流地址给登录后的浏览器，此时程序会自动在 Chrome 打开直播间，由插件送回地址；需要已安装插件并登录 TikTok
...
0.4.0 起插件在观众评论之外多了一个职责：对程序自己解析不出来的直播间，插件会监听直播页自身的网络请求，抓到播放器实际使用的流地址后送回程序。这需要 manifest 里新增的 `webRequest` 权限和 TikTok CDN 相关的 host 权限——如果你在这之前就装过插件，请到 `chrome://extensions` 点一次「重新加载」，让新的 manifest 和后台脚本生效。
