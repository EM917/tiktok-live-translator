...
- 🔄 **抗故障** —— 五条独立的流地址解析路径（TikTok 官方接口 → 系统 WebKit 引擎在后台加载直播页（macOS，TikTok 只把某些房间的流地址给真正的浏览器时靠它，实测 2 秒）→ yt-dlp → yt-dlp 借用浏览器登录态 → 直播页解析），因为 yt-dlp 提取器被拦截时会将失败报告为「未开播」。每个解析结果在会话开始前先行验证。断流后以重新解析的地址重连，并区分网络中断与直播结束；识别落后时自动丢段保实时；后台保持 yt-dlp 为最新版。TikTok 暂时不给某个直播间的流地址时（多半是短时间内请求过多被限流），程序会隔 20 秒自动重试几次，仍不行就明确提示稍后再试
