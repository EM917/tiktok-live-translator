...
试过但**没有上线**的一条，记在这里免得再提：「凭空欠债」——中文出现「欠/债/
欠款」而西语没有 deber / deuda。动机是实录里一句
    te está quedando en 47 dólares → 你现在只欠了47美元
看着是模型凭空加了一层债务关系。但复查原文，Whisper 输出的是
    ...te está quedando en 47 dólares y tú debes 47
`debes` 就在源文里，译文是忠实的——那是 ASR 把话听乱了，不是翻译加料。
规则写出来后在 600 条标注集和 1204 条真实译文上各标记 0 条：**零证据**。
唯一的动机案例经不起复查，规则就不该上线。
